...
4. Pick `design.background`: **prefer `shader` (v11 #2) — a real GPU-rendered lit gradient-flow field in the project's own colors. It reads premium (genuine depth + light, not flat canvas), and it does double duty: it lifts the frame brightness and keeps constant motion, so it clears the brightness and dead-air gates by construction (the 2026-07-05 test went mean-luma 37→111, 14%→100% bright scenes, dead-air 0.20→0.04s).** Alternatives, rotated per the no-repeat rule: `aurora` (accent-derived blobs) for dark native palettes, `starfield` for library/preset dark, `grid` for infra stories, `none` for light editorial. Don't repeat yesterday's background style. (`shader` needs WebGL, which the recorder's headless Chromium has; it self-tunes to hold frame rate.)
